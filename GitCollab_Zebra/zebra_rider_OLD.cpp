...
//				tempRiderPid.r_idalpha = read_from_file[i].r_idalpha;
//				tempRiderPid.r_idnum = read_from_file[i].r_idnum;
...
//	Rider_pid temp_pid;
//	vector <Rider_pid> temp_retrievePID_vector;
//
//	system("cls");
//	//_________________________________________________________________________________________________________________________________________________________________
//	//*****************************************************************************************************************************************************************
//	//This section pulls in all important details for the logged in rider
//	temp_retrievePID_vector = rider_retrieve_info();
//	cout << "\nDebugging: Checking that temp_retrieve_info is passed " << temp_retrievePID_vector[1].r_pname;
//	cout << "\nDebugging: Checking what the temp_retrieve_info file size is " << temp_retrievePID_vector.size();
//	cout << "\nDebugging: Is the riderPID UID passed?  " << uid.r_idalpha << " and " << uid.r_idnum;
//	for (int i = 0; i < temp_retrievePID_vector.size(); i++)
//	{
//		if (temp_retrievePID_vector[i].r_idalpha == uid.r_idalpha && temp_retrievePID_vector[i].r_idnum == uid.r_idnum)
//		{
//			cout << "\nThe default location stored in the file at this time is: \t" << temp_retrievePID_vector[i].r_defaultloc;
//			//For the passed user, i store a copy of their UID, pname, and default loc into both the temp_nopid and temp_pid structures. 
//			//This allows me to pass on the UID easily while maintaining customer privacy
//			//However this also means that i have to update both noPID AND PID structs everytime there's a change in defaultloc/pname (unlikely)
//			temp_nopid.rr_UIDalpha = temp_retrievePID_vector[i].r_idalpha;
//			temp_nopid.rr_UIDnum = temp_retrievePID_vector[i].r_idnum;
//			temp_pid.r_idalpha = temp_retrievePID_vector[i].r_idalpha;
//			temp_pid.r_idnum = temp_retrievePID_vector[i].r_idnum;
//			temp_nopid.rr_pname = temp_retrievePID_vector[i].r_pname;
//			temp_pid.r_pname = temp_retrievePID_vector[i].r_pname;
//			temp_nopid.rr_defaultloc = temp_retrievePID_vector[i].r_defaultloc;
//			temp_pid.r_defaultloc = temp_retrievePID_vector[i].r_defaultloc;
//
//			//Checking default location in pid file, then propagating it to the temp_nopid file
//			string retrievedloc = 
//			temp_pid = has_defaultloc(temp_pid);
//			temp_nopid.rr_defaultloc = temp_pid.r_defaultloc;
//			 
//
//
//			cout << "\nPreferred name and transferred temp_nopid default loc are " << temp_nopid.rr_pname << " and " << temp_nopid.rr_defaultloc;
//			cout << "\nCheck what temp_retrieve_info[i].r_card_num value " << temp_retrievePID_vector[i].r_card_num;
//			//Desensitising payment details by checking if riderpid file has payment details. If yes, pay_avail = "Yes", else "No"
//			if (temp_retrievePID_vector[i].r_card_num == "nil" || temp_retrievePID_vector[i].r_card_num == "")
//			{
...
//		
//		searchAndUpdate_defaultloc(temp_pid, temp_retrievePID_vector);
...
//				temp_pid = pay_details(temp_pid, temp_retrievePID_vector);
//				temp_nopid.rr_pay_avail = "Yes";
//			}
//			cout << "\n Are payment details stored? " << temp_nopid.rr_pay_avail;
...
//		}
//	}
//	rider_main(temp_retrievePID_vector);
//	//return rselect;
//}
//
//void disp_fares_charges()
...
//
////#######################################################
////  CHECK IF STRUCT VAR HAS STORED DATA
////  has_defaultloc(Riderstore struct checkloc)
////#######################################################
//
////This function searches within the PID file to see if a specific rider has a stored default location
//Rider_pid has_defaultloc(Rider_pid checkloc)
//{
//	vector <Rider_pid> temp_retrievePID_vector;
//	temp_retrievePID_vector = rider_retrieve_info();
//	cout << "\nDEBUGGING: Checking that checkloc's uida and uidn are passed " << checkloc.r_idalpha << checkloc.r_idnum;
//	for (int i = 0; i < temp_retrievePID_vector.size(); i++)
//	{
//		if (temp_retrievePID_vector[i].r_idalpha == checkloc.r_idalpha && temp_retrievePID_vector[i].r_idnum == checkloc.r_idnum)
//		{
//			//if pid database has the rider's default location, pull it over to the temp pid struct. Else update rider's pid default location with details in temp pid struct
//			if (temp_retrievePID_vector[i].r_defaultloc == "")
//			{
//				temp_retrievePID_vector[i].r_defaultloc = checkloc.r_defaultloc;
//				checkloc.r_defaultloc = temp_retrievePID_vector[i].r_defaultloc;
//				cout << "\nDebugging: pid database was EMPTY and is now filled with " << temp_retrievePID_vector[i].r_defaultloc;
//			}
//			else
//			{
//				checkloc.r_defaultloc = temp_retrievePID_vector[i].r_defaultloc;
//				cout << "\nDebugging: pid database was NOT emmpty and has passed (to checkloc) the defaultloc " << checkloc.r_defaultloc;
//			}
//			
//			
//		}
